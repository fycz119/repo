package com.lagou.educourseboot.service.impl;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.lagou.educourseboot.entity.*;
import com.lagou.educourseboot.mapper.*;
import com.lagou.educourseboot.remote.OrderRemoteService;
import com.lagou.educourseboot.service.CourseService;
import org.springframework.beans.BeanUtils;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.redis.core.RedisTemplate;
import org.springframework.data.redis.serializer.RedisSerializer;
import org.springframework.data.redis.serializer.StringRedisSerializer;
import org.springframework.stereotype.Service;

import java.util.ArrayList;
import java.util.List;
import java.util.concurrent.TimeUnit;

/**
 * @BelongsProject: edu-lagou
 * @Author: GuoAn.Sun
 * @CreateTime: 2021-03-03 15:43
 * @Description:
 */
@Service
public class CourseServiceImpl implements CourseService {
    @Autowired
    private CourseMapper courseMapper;
    @Autowired
    private TeacherMapper teacherMapper;
    @Autowired
    private LessonMapper lessonMapper;
    @Autowired
    private SectionMapper sectionMapper;
    @Autowired
    private MediaMapper mediaMapper;

    @Autowired
    private RedisTemplate<Object,Object> redisTemplate;

    @Autowired
    private OrderRemoteService orderRemoteService;

    @Override
    public List<CourseDTO> getAllCourse() {
        //将redis内存中的序列化的集合名称用String重新命名（增加可读性）
        RedisSerializer rs = new StringRedisSerializer();
        redisTemplate.setKeySerializer(rs);
        // 1、先去redis中查询
        System.out.println("**先去redis中查询**");
        // 课程dto集合
        List<CourseDTO> courseDTOS = (List<CourseDTO>) redisTemplate.opsForValue().get("allCourses");
        // 2、redis中没有，才回去mysql查询
        if (null == courseDTOS) {
            synchronized (this) {
                courseDTOS = (List<CourseDTO>) redisTemplate.opsForValue().get("allCourses");
                if (null == courseDTOS) {
                    System.out.println("==查询redis==");
                    // 查询全部课程
                    List<Course> initCourse = getInitCourse(null);
                    courseDTOS = new ArrayList<>();
                    for (Course course : initCourse) {
                        CourseDTO dto = new CourseDTO();
                        // course将属性全部赋给给courseDTO对象
                        BeanUtils.copyProperties(course, dto);
                        courseDTOS.add(dto);
                        // 设置老师
                        setTeacher(dto);
                        // 设置前两节课
                        setTop2Lesson(dto);
                    }
                    redisTemplate.opsForValue().set("allCourses", courseDTOS, 10, TimeUnit.MINUTES);
                }
            }
        }
        return courseDTOS;
    }

    @Override
    public CourseDTO getCourseById(Integer courseid) {
        // 获取课程基本信息
        Course course = this.courseMapper.selectOne(new QueryWrapper<Course>().eq("id", courseid));
        CourseDTO courseDTO = new CourseDTO();
        BeanUtils.copyProperties(course, courseDTO);
        setTeacher(courseDTO);
        // 关联章节
        List<SectionDTO> courseSection = getCourseSection(courseDTO);
        courseDTO.setCourseSections(courseSection);
        return courseDTO;
    }

    @Override
    public List<CourseDTO> getCoursesByUserId(Integer userid) {
        // 根据用户id获得已购买课程id集合
        List<Object> ids = orderRemoteService.getOKOrderCourseIds(userid);
        // 根据课程id集合查询相应的课程信息集合
        return this.getMyCourses(ids);
    }

    // 查询已购买课程
    @Override
    public List<CourseDTO> getMyCourses(List<Object> ids) {
        //将redis内存中的序列化的集合名称用String重新命名（增加可读性）
        RedisSerializer rs = new StringRedisSerializer();
        redisTemplate.setKeySerializer(rs);
        // 1、先去redis中查询
        System.out.println("**先去redis中查询**");
        // 课程dto集合
        List<CourseDTO> courseDTOS = (List<CourseDTO>) redisTemplate.opsForValue().get("myCourses");
        // 2、redis中没有，才回去mysql查询
        if (null == courseDTOS) {
            synchronized (this) {
                courseDTOS = (List<CourseDTO>) redisTemplate.opsForValue().get("myCourses");
                if (null == courseDTOS) {
                    System.out.println("==查询redis==");
                    // 查询全部课程
                    List<Course> initCourse = getInitCourse(ids);
                    courseDTOS = new ArrayList<>();
                    for (Course course : initCourse) {
                        CourseDTO dto = new CourseDTO();
                        // course将属性全部赋给给courseDTO对象
                        BeanUtils.copyProperties(course, dto);
                        courseDTOS.add(dto);
                        // 设置老师
                        setTeacher(dto);
                        // 设置前两节课
                        setTop2Lesson(dto);
                    }
                    redisTemplate.opsForValue().set("allCourses", courseDTOS, 10, TimeUnit.MINUTES);
                }
            }
        }
        return courseDTOS;
    }

    // 初始化基本的全部课程
    public List<Course> getInitCourse(List<Object> ids) {
        QueryWrapper q = new QueryWrapper();
        q.eq("status", 1);// 已上架
        q.eq("is_del", Boolean.FALSE);// 未删除
        q.orderByDesc(" sort_num ");// 排序
        if (ids != null) {
            q.in("id", ids);
        }
        return this.courseMapper.selectList(q);
    }

    // 基本的老师查询
    public void setTeacher(CourseDTO courseDTO) {
        QueryWrapper q = new QueryWrapper();
        q.eq("course_id", courseDTO.getId());
        q.eq("is_del", Boolean.FALSE);
        Teacher teacher = this.teacherMapper.selectOne(q);
        courseDTO.setTeacher(teacher);
    }

    // 前两节课
        private void setTop2Lesson(CourseDTO courseDTO) {
        QueryWrapper q = new QueryWrapper();
        q.eq("course_id", courseDTO.getId());
        q.eq("is_del", Boolean.FALSE);
        q.orderByAsc("section_id", "order_num");
        q.last("limit 0, " + 2);
        List<CourseLesson> list = this.lessonMapper.selectList(q);
        courseDTO.setLessonsDTO2(list);
    }

    // 获取章节
    private List<SectionDTO> getCourseSection(CourseDTO courseDTO) {
        List<SectionDTO> sectionDTOS = new ArrayList<>();
        // 获取Section List
        QueryWrapper sectionMapperQueryWrapper = new QueryWrapper<>();
        sectionMapperQueryWrapper.eq("course_id", courseDTO.getId());
        sectionMapperQueryWrapper.eq("is_del", Boolean.FALSE);
        sectionMapperQueryWrapper.eq("status", 2);// 已发布
        sectionMapperQueryWrapper.orderByAsc("order_num"); //排序

        List<CourseSection> courseSections = this.sectionMapper.selectList(sectionMapperQueryWrapper);
        //视频
        // 循环Section获取对应lesson
        for (CourseSection section : courseSections) {
            SectionDTO sectionDTO = new SectionDTO();
            BeanUtils.copyProperties(section, sectionDTO);

            // 获取对应课程
            QueryWrapper lessonQ= new QueryWrapper();
            lessonQ.eq("section_id", section.getId());
            lessonQ.eq("is_del", Boolean.FALSE);
            lessonQ.orderByAsc("order_num");
            List<CourseLesson> courseLessons = this.lessonMapper.selectList(lessonQ);

            // lessons放入lessonDtos，加上视频
            List<LessonDTO> lessonDTOS = new ArrayList<>();
            for (CourseLesson lesson : courseLessons) {
                LessonDTO lessonDTO = new LessonDTO();
                BeanUtils.copyProperties(lesson, lessonDTO);
                // 设置每节课对应视频
                setMedia(lessonDTO);
                lessonDTOS.add(lessonDTO);
            }

            // 将课程放入Section
            sectionDTO.setCourseLessons(lessonDTOS);
            // 整个章节放入章节集合
            sectionDTOS.add(sectionDTO);
        }

        return sectionDTOS;
    }

    // 设置每节课对应视频
    private void setMedia(LessonDTO lessonDTO){
        QueryWrapper mediaQ = new QueryWrapper();
        mediaQ.eq("lesson_id", lessonDTO.getId());
        mediaQ.eq("is_del", Boolean.FALSE);
        CourseMedia courseMedia = this.mediaMapper.selectOne(mediaQ);
        lessonDTO.setCourseMedia(courseMedia);
    }

}
